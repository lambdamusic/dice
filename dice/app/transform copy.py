...
def prune_concepts(concepts_and_pubs, min_score=None, min_freq=None, verbose=True):
    ...
    """
    if verbose: click.secho(f"Pruning concepts data...", fg="green")

    if verbose: click.secho(f"MIN score= {str(min_score)} MIN freq= {str(min_freq)}", dim=True)
    ...
    c = concepts_and_pubs.drop_duplicates("concept")[['concept', 'frequency', 'score_avg']]
...
def dsl_to_networkx(concepts_and_pubs, min_edge_weight=None, verbose=True):
    ...
    if verbose: click.secho(f"Generating networkX graph...", fg="green")
    ...
    if verbose: click.secho(f".. adding nodes.. ")
    #
    ...
    #
    if verbose: click.secho(f".. adding edges from pubs cooccurrence.. ")
